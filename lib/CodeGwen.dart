...
""";
  ...
  Future<String> _callGwen(String code) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          "Ollama (Qwen) error: ${response.statusCode} ${response.body}");
    }
  }
  ...
  Future<void> _improveCode() async {
    ...
    try {
      ...
      // strip ``` if model adds them
      if (improvedCodeText.startsWith('```')) {
        final firstNl = improvedCodeText.indexOf('\n');
        if (firstNl != -1) {
          improvedCodeText = improvedCodeText.substring(firstNl + 1);
        }
        final lastTrip = improvedCodeText.lastIndexOf('```');
        if (lastTrip != -1) {
          improvedCodeText = improvedCodeText.substring(0, lastTrip);
        }
  ...
  Future<void> _saveFile() async {
    if (_currentFilePath == null || _currentFilePath!.isEmpty) {
      ...
      _currentFileName =
      !kIsWeb ? savePath.split(Platform.pathSeparator).last : savePath;
    }
